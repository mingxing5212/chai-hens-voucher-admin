	...
	<div class="dataTables_paginate paging_bootstrap">
		<ul class="pagination">
			...
			<c:choose>
				<c:when test="${seed.hasPreviousPage}">
					<li data-lp="1" class="prev">
						<a href="${ctx}/${seed.actionPath}?${pageSize}&page=1${queryStr}${pageOrder}"><icon class="fa fa-angle-double-left"></icon></a>
					</li>
				</c:when>
				<c:otherwise>
			...
			<c:choose>
				<c:when test="${seed.hasPreviousPage}">
					<li data-lp="1" class="prev">
						<a href="${ctx}/${seed.actionPath}?${pageSize}&page=${seed.pageNumber-1}${queryStr}${pageOrder}"><icon class="fa fa-angle-left"></icon></a>
					</li>
				</c:when>
				<c:otherwise>
					<li data-lp="1" class="prev disabled">
			...
			<c:choose>
				<c:when test="${seed.pageNumber + 1 > count}">
					<c:forEach begin="1" end="${count-1}">
						<c:if test="${count>0}">
							...
							<li data-lp="${cur}">
								<a href="${ctx}/${seed.actionPath}?${pageSize}&page=${cur}${queryStr}${pageOrder}">${cur}</a>
							</li>
				...
				<c:otherwise>
					<c:if test="${seed.pageNumber > 1}">
						<c:forEach begin="1" end="${seed.pageNumber - 1}" varStatus="status">
							<li data-lp="${status.count}">
								<a href="${ctx}/${seed.actionPath}?${pageSize}&page=${status.count}${queryStr}${pageOrder}">${status.count}</a>
							</li>
						</c:forEach>
					</c:if>
				</c:otherwise>
			...
			<!-- current page -->
			<li data-lp="${seed.pageNumber}" class="active"><a href="javascript:void(0);">${seed.pageNumber}</a></li>
			...
			<c:forEach begin="1" end="${count-1}" varStatus="status">
				<c:if test="${_break == '0'}">
					...
					<c:choose>
						...
						<c:otherwise>
							<li data-lp="${cur}"><a href="${ctx}/${seed.actionPath}?${pageSize}&page=${cur}${queryStr}${pageOrder}">${cur}</a></li>
						</c:otherwise>
					</c:choose>
				</c:if>
			</c:forEach>
			...
			<c:choose>
				<c:when test="${seed.pageNumber < seed.totalPages}">
					<li data-lp="${seed.totalPages}" class="next">
						<a href="${ctx}/${seed.actionPath}?${pageSize}&page=${seed.pageNumber+1}${queryStr}${pageOrder}"> <icon class="fa fa-angle-right"></icon></a>
					</li>
				</c:when>
				<c:otherwise>
			...
			<c:choose>
				<c:when test="${seed.pageNumber < seed.totalPages}">
					<li data-lp="${seed.totalPages}" class="next">
						<a href="${ctx}/${seed.actionPath}?${pageSize}&page=${seed.totalPages}${queryStr}${pageOrder}"><icon class="fa fa-angle-double-right"></icon></a>
					</li>
				</c:when>
				<c:otherwise>
					<li data-lp="7" class="next disabled">
