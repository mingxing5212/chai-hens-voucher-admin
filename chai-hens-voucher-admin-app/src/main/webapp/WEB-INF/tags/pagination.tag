...
<c:if test="${seed != null && !empty seed.result}">
	...
	<c:set var="pageSizes" value="${fn:split('5,10,25,50,100', ',')}"  scope="request"/>
	<c:set var="queryStr" value="${seed.queryStr}"  scope="request"/>
	
	<c:choose>
		<c:when test="${seed.pageOrder.name != ''}">
			<c:set var="pageOrder" value="&pageOrder.order=${seed.pageOrder.orderExpressions}"   scope="request"/>
		</c:when>
		<c:otherwise>
						...
						<c:choose>
							<c:when test="${seed.totalSize < seed.endPosition}">
								<label>
                                     	 ...
                                     	 显示第 ${seed.startPosition+1} 至  ${seed.totalSize} 项结果，共  ${seed.totalSize} 项
                                </label>
							</c:when>
							<c:otherwise>
                                <label>
                                   
                                  	  显示第 ${seed.startPosition+1} 至  ${seed.endPosition} 项结果，共  ${seed.totalSize} 项
                                </label>
							</c:otherwise>
						</c:choose>
					</div>
...
<c:if test="${(seed == null || empty seed.result) && position eq 'below'}">
	<div class="row"><div class="col-md-12" style="text-align:center;"></div></div>
</c:if>